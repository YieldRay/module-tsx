...
import { PopoverHolder, Scrim, Select, TooltipHolder } from "soda-material";
    ...
    <Card disabled style={{ padding: "1rem", display: "inline-block" }}>
      ...
      <Flex flexDirection="column" gap=".5rem">
        ...
        <Flex gap="2rem" justifyContent="space-between">
          ...
          <TextField labelText="Comment" variant="outlined" textarea />
        </Flex>
      </Flex>
        ...
        <div style={{ flex: "1" }}>
          {[undefined, 0.4, 0.3, 0.2, 0.1].map((value, index) => (
            <Flex gap="4px" key={index}>
              <span>{5 - index}</span>
              <LinearProgressIndicator thickness="6px" value={value} />
            </Flex>
          ))}
        </div>
      </Flex>
