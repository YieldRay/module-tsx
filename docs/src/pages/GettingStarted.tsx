  ...
  return (
    <article>
      ...
      <p className="text-lg leading-relaxed text-[var(--md-sys-color-on-surface-variant)]">
        <strong>module-tsx</strong> lets you run TypeScript and React directly
        in the browser — no build step, no npm install, no bundler required. You
        publish source code and users run it instantly.
      </p>
      ...
      <p>
        ...
        <code>{`<script type="module-tsx">`}</code> tag, fetches and transforms
        the TypeScript/JSX source using the TypeScript compiler running in the
        browser, then executes the result as an ES module via a Blob URL.
      </p>
      <p>
        Bare specifiers like <code>import React from "react"</code> are
        automatically resolved to <code>https://esm.sh/react</code>. You can use
        any npm package without installing anything.
      </p>
    </article>
  );
}
